...
#pragma omp parallel for private(elt)
   for (elt = 0; elt < batchSize; elt++) {
      ...
      for (i = 0; i < nInputPlane; i++) {
         for (k = 0; k < kL; k++) {
            THVector_(cadd)(output_t->storage->data + output_t->storageOffset + output_t->stride[0]*i,
                           output_t->storage->data + output_t->storageOffset + output_t->stride[0]*i,
...
static int nnconv1d_(VerticalConvolution_updateGradInput)(lua_State *L)
{
   THTensor *input = luaT_checkudata(L, 2, torch_Tensor);
...
static int nnconv1d_(VerticalConvolution_accGradParameters)(lua_State *L)
{
   THTensor *input = luaT_checkudata(L, 2, torch_Tensor);
   THTensor *gradOutput = luaT_checkudata(L, 3, torch_Tensor);
...
static const struct luaL_Reg nnconv1d_(VerticalConvolution__) [] = {
  {"VerticalConvolution_updateOutput", nnconv1d_(VerticalConvolution_updateOutput)},
  {"VerticalConvolution_updateGradInput", nnconv1d_(VerticalConvolution_updateGradInput)},
  {"VerticalConvolution_accGradParameters", nnconv1d_(VerticalConvolution_accGradParameters)},
  {NULL, NULL}
};


static void nnconv1d_(VerticalConvolution_init)(lua_State *L)
{
  luaT_pushmetatable(L, torch_Tensor);
  luaT_registeratname(L, nnconv1d_(VerticalConvolution__), "nn");
  lua_pop(L,1);
}
