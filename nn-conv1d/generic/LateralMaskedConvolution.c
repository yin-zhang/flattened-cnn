...
#define TH_GENERIC_FILE "generic/LateralMaskedConvolution.c"
#else


static int nnconv1d_(LateralMaskedConvolution_updateOutput)(lua_State *L)
{
   ...
   THTensor *maskedWeights = THTensor_(new)();
   THTensor_(cmul)(maskedWeights, weight, mask);
...
#pragma omp parallel for private(elt)
   for (elt = 0; elt < batchSize; elt++) {
      ...
      // convolve
      THTensor_(addmm)(output2d, 1, output2d, 1, maskedWeights, input2d);
...
static int nnconv1d_(LateralMaskedConvolution_updateGradInput)(lua_State *L)
{
   THTensor *input = luaT_checkudata(L, 2, torch_Tensor);
   ...
   THTensor *maskedWeights = THTensor_(new)();
   THTensor_(cmul)(maskedWeights, weight, mask);
   THTensor_(transpose)(maskedWeights, maskedWeights, 0, 1);
...
#pragma omp parallel for private(elt)
   for (elt = 0; elt < batchSize; elt++) {
      ...
      // convolve
      THTensor_(addmm)(gradInput2d, 0, gradInput2d, 1, maskedWeights, gradOutput2d);
...
static int nnconv1d_(LateralMaskedConvolution_accGradParameters)(lua_State *L)
{
   THTensor *input = luaT_checkudata(L, 2, torch_Tensor);
   THTensor *gradOutput = luaT_checkudata(L, 3, torch_Tensor);
...
static const struct luaL_Reg nnconv1d_(LateralMaskedConvolution__) [] = {
  {"LateralMaskedConvolution_updateOutput", nnconv1d_(LateralMaskedConvolution_updateOutput)},
  {"LateralMaskedConvolution_updateGradInput", nnconv1d_(LateralMaskedConvolution_updateGradInput)},
  {"LateralMaskedConvolution_accGradParameters", nnconv1d_(LateralMaskedConvolution_accGradParameters)},
  {NULL, NULL}
};

static void nnconv1d_(LateralMaskedConvolution_init)(lua_State *L)
{
  luaT_pushmetatable(L, torch_Tensor);
  luaT_registeratname(L, nnconv1d_(LateralMaskedConvolution__), "nn");
  lua_pop(L,1);
}
