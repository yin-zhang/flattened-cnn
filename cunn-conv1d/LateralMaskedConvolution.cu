...
static int cunnconv1d_LateralMaskedConvolution_updateOutput(lua_State *L) {
   ...
   THCudaTensor *maskedWeights = THCudaTensor_new(state);
   THCudaTensor_cmul(state, maskedWeights, weight, mask);
   ...
   for (int elt = 0; elt < batchSize; elt ++) {
      ...
      THCudaBlas_gemm(
         ...
         THCudaTensor_data(state, maskedWeights), nInputPlane,
         1,
         THCudaTensor_data(state, output_n), outputHeight*outputWidth
      );
   }
   ...
   THCudaTensor_free(state, maskedWeights);
...
static int cunnconv1d_LateralMaskedConvolution_updateGradInput(lua_State *L) {
   THCState *state = getCutorchState(L);
   THCudaTensor *input = (THCudaTensor *)luaT_checkudata(L, 2, "torch.CudaTensor");
...
static int cunnconv1d_LateralMaskedConvolution_accGradParameters(lua_State *L) {
   THCState *state = getCutorchState(L);
   THCudaTensor *input = (THCudaTensor *)luaT_checkudata(L, 2, "torch.CudaTensor");
   THCudaTensor *gradOutput = (THCudaTensor *)luaT_checkudata(L, 3, "torch.CudaTensor");
...
static const struct luaL_Reg cunnconv1d_LateralMaskedConvolution__ [] = {
   {"LateralMaskedConvolution_updateOutput", cunnconv1d_LateralMaskedConvolution_updateOutput},
   {"LateralMaskedConvolution_updateGradInput", cunnconv1d_LateralMaskedConvolution_updateGradInput},
   {"LateralMaskedConvolution_accGradParameters", cunnconv1d_LateralMaskedConvolution_accGradParameters},
   {NULL, NULL}
};


void cunnconv1d_LateralMaskedConvolution_init(lua_State *L)
{
   luaT_pushmetatable(L, "torch.CudaTensor");
   luaT_registeratname(L, cunnconv1d_LateralMaskedConvolution__, "nn");
   lua_pop(L,1);
}
