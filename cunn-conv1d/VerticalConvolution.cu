...
__global__ void im2col_kernel_v(const int n, const float* data_im,
...
__global__ void conv_vertical_naive_output(const int n, float *y,
...
__global__ void conv_vertical_naive_gradInput(const int n, float *dx,
...
/*
__global__ void conv_vertical_naive_gradParam(const int n, float *dw,
...
__global__ void conv_vertical_naive_gradWeight(const int n, float *y,
...
static int cunnconv1d_VerticalConvolution_updateOutput(lua_State *L) {
   ...
   for (int elt = 0; elt < batchSize; elt ++) {
      ...
      conv_vertical_naive_output <<<GET_BLOCKS(num_threads), CUDA_NUM_THREADS>>>
         (num_threads,
...
static int cunnconv1d_VerticalConvolution_updateGradInput(lua_State *L) {
   THCState *state = getCutorchState(L);
   THCudaTensor *input = (THCudaTensor *)luaT_checkudata(L, 2, "torch.CudaTensor");
   ...
   for (int elt = 0; elt < batchSize; elt ++) {
      ...
      conv_vertical_naive_gradInput <<<GET_BLOCKS(num_threads), CUDA_NUM_THREADS>>>
         (num_threads,
...
static int cunnconv1d_VerticalConvolution_accGradParameters(lua_State *L) {
   THCState *state = getCutorchState(L);
   THCudaTensor *input = (THCudaTensor *)luaT_checkudata(L, 2, "torch.CudaTensor");
   THCudaTensor *gradOutput = (THCudaTensor *)luaT_checkudata(L, 3, "torch.CudaTensor");
   ...
   for (int elt = 0; elt < batchSize; elt ++) {
      ...
      im2col_kernel_v <<<GET_BLOCKS(num_threads), CUDA_NUM_THREADS>>> (
         num_threads,
         THCudaTensor_data(state, input_n),
         inputHeight, inputWidth, kL, 1, 0, 0, 1, 1,
   ...
   conv_vertical_naive_gradWeight <<<GET_BLOCKS(num_threads_), CUDA_NUM_THREADS>>> (
      num_threads_,
...
static const struct luaL_Reg cunnconv1d_VerticalConvolution__ [] = {
   {"VerticalConvolution_updateOutput", cunnconv1d_VerticalConvolution_updateOutput},
   {"VerticalConvolution_updateGradInput", cunnconv1d_VerticalConvolution_updateGradInput},
   {"VerticalConvolution_accGradParameters", cunnconv1d_VerticalConvolution_accGradParameters},
   {NULL, NULL}
};


void cunnconv1d_VerticalConvolution_init(lua_State *L)
{
   luaT_pushmetatable(L, "torch.CudaTensor");
   luaT_registeratname(L, cunnconv1d_VerticalConvolution__, "nn");
   lua_pop(L,1);
}
